...
assert(reportWxml.includes("生成并保存"), "报告页底部应包含生成并保存按钮");
assert(reportWxml.includes("保存到相册"), "报告页底部应包含保存到相册状态");
assert(reportWxml.includes("预览图片"), "报告页底部应包含预览图片按钮");
assert(reportWxml.includes("转发好友"), "报告页底部应包含转发好友按钮");
...
assert(reportWxss.includes("pointer-events: none"), "报告导出画布不应阻塞页面点击");
